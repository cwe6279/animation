...
talker/
  ...
  faces/
    ...
    goat/  dragon/       <- live textured eyes (pupils move and dilate) from Adafruit Uncanny Eyes designs (MIT)
    green_cat/eye/       <- the cat's own eye art unwrapped into live parts by faces/tools/unwrap_eye.py
    tools/unwrap_eye.py  <- turn any flat eye PNG into textured-eye parts (iris strip, pupil map, lids, highlight)
  textured_eye.py        <- eye motion model + per-frame compositor for textured eyes
...
- **Making your own eye art live**: `python faces/tools/unwrap_eye.py faces/<name>/eye_right.png faces/<name>/eye`
  ...
  detected one is off. The cat was converted this way.
